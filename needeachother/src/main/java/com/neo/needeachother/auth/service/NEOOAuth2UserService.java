...
public class NEOOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    /**
     * DefaultOAuth2UserService 객체를 생성해 loadUser() 메소드를 위임 호출합니다. <br>
     * DefaultOAuth2UserService의 loadUser()는 소셜 로그인 API의 사용자 정보 제공 URI로 요청을 보내서
     * 사용자 정보를 얻은 후, 이를 통해 DefaultOAuth2User 객체를 생성 후 반환합니다.<br>
     * 결과적으로, OAuth2User는 OAuth 서비스에서 가져온 유저 정보를 담고 있는 유저를 의미합니다.
     */
    @Override
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        OAuth2User oAuth2UserFromResourceServer = delegate.loadUser(userRequest);

        return createNeoCustomOAuth2UserFromResourceServer(userRequest, oAuth2UserFromResourceServer);
    }
     ...
     */
    private NEOCustomOAuth2User createNeoCustomOAuth2UserFromResourceServer(OAuth2UserRequest userRequest, OAuth2User oAuth2UserFromResourceServer){
        // request로부터 provider 타입 확보
        ...
        Map<String, Object> attributes = oAuth2UserFromResourceServer.getAttributes();
        ...
        // 새로운 회원 가입 혹은 기존 회원 정보 확보
        NEOUserEntity createdUser = getUser(extractAttributes, providerType);
