...
public class NEOCreateStarInfoRequest {
    ...
    @NotBlank(message = "본인의 이름은 필수 입력사항입니다.")
    ...
    @Pattern(regexp = "^(010|011|016|017|018|019)-?(?:\\d{3,4})-?\\d{4}$", message = "입력한 핸드폰 번호의 형식이 유효하지 않습니다.")
    @Schema(description = "핸드폰 번호", example = "010-0000-0000")
    @JsonProperty(value = "phone_number", required = true)
    private String phoneNumber;

    /* 네오에서 사용할 닉네임 */
    @NotBlank(message = "네오에서 사용하는 닉네임은 필수로 입력해야 합니다.")
    ...
    /* 스타의 실제 활동명 */
    @NotBlank(message = "내가 스타로서 활동하고 있는 활동명 입력은 스타 가입에 필수로 입력해야 합니다.")
    ...
    /* 스타 구분, 중복 선택 가능 */
    @NotEmpty(message = "스타 구분을 선택하지 않았습니다.")
    @Schema(description = """
    ...
    @JsonIgnore
    private List<String> wrongClassificationCodeList;
}
